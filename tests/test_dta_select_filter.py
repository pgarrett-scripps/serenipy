...
class TestDtaSelectFilter(unittest.TestCase):
    def test_to_from_dta_select_filter_V2_1_13(self):
        with open(DATA_DIR / "DTASelect-filter_V2_1_13.txt", "r") as file:
            version, head_lines, dta_select_filter_results, tail_lines = from_dta_select_filter(
                file
            )

        self.assertEqual(
            dta_select_filter_results[0].protein_lines[0].locus_name, "sp|P05387|RLA2_HUMAN"
        )
        ...
        self.assertEqual(
            dta_select_filter_results[0].protein_lines[0].description_name,
        ...
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].file_name,
            "190806_300ng_180m_03_Slot2-3_1_646_nopd.357198.357198.2",
        )
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].file_path,
        ...
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].mass_plus_hydrogen, 1868.9581
        )
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].calc_mass_plus_hydrogen, 1868.9752
        )
        ...
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].predicted_im_value, 1.1519999504089355
        )
        self.assertEqual(dta_select_filter_results[0].peptide_lines[0].im_score, 0.5299)
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].sequence, "R.YVASYLLAALGGNSSPSAK.D"
        )

        dta_select_filter_str = to_dta_select_filter(
            version, head_lines, dta_select_filter_results, tail_lines
        )
        version, head_lines, dta_select_filter_results, tail_lines = from_dta_select_filter(
            dta_select_filter_str
        )
    ...
    def test_to_from_dta_select_filter_V2_1_12(self):
        ...
        self.assertEqual(
            dta_select_filter_results[0].protein_lines[0].locus_name, "sp|P04792|HSPB1_HUMAN"
        )
        ...
        self.assertEqual(
            dta_select_filter_results[0].peptide_lines[0].calc_mass_plus_hydrogen, 831.50867
        )
    ...
    def test_dtafilter_filter(self):
        peptides_lines = [
            PeptideLine(file_name="file1.1.1.2", sequence="PEPTIDE", conf=0.9, x_corr=1),
            PeptideLine(file_name="file2.1.1.2", sequence="PEPTIDE", conf=0.8, x_corr=1),
            PeptideLine(file_name="file2.1.1.2", sequence="PEPTIDE", conf=0.7, x_corr=1),
            PeptideLine(file_name="file4.1.1.4", sequence="PEPTIDES", conf=0.7, x_corr=1),
        ]

        result = DTAFilterResult(protein_lines=None, peptide_lines=peptides_lines)
